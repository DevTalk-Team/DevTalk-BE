...
public class KafkaConsumer {
    ...
    public void receiveConsultationInfo(String kafkaMessage) {
        ...
        //*** 메시지를 받을 형태를 선언해주세요 ***//
        ProductReservedDetails productReservedDetails = null;
        try{
            // 카프카로 받은 메시지를 객체로 변환하기 위한 과정
            //*** 받은 String형 메시지를 역직렬화 합니다. 받을 타입을 두번째 매개변수에 추가해주세요 ***//
            productReservedDetails = deserializeMapper().readValue(kafkaMessage, ProductReservedDetails.class);
        } catch (JsonProcessingException ex){
            ex.printStackTrace();
        }
        //*** 받은 메시지로 수행할 행동을 아래에 써주세요***//
        dataSynchronization(productReservedDetails);
        //**********************************************//
    }

    // *** 수행할 행동 *** //
    private void dataSynchronization(ProductReservedDetails productReservedDetails) {
