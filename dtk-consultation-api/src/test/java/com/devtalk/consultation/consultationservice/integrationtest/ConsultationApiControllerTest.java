...
import com.devtalk.consultation.consultationservice.consultation.adapter.out.persistence.LinkItemQueryRepo;
...
import com.devtalk.consultation.consultationservice.integrationtest.setup.LinkItemSetUp;
...
class ConsultationApiControllerTest {
    ...
    @Autowired private LinkItemQueryableRepo linkItemQueryableRepo;

    @Autowired private MemberSetUp memberSetup;
    @Autowired private LinkItemSetUp linkItemSetUp;

    @BeforeAll
    void setUp() {
        productServiceClient = Mockito.mock(ProductServiceClient.class);
        ConsultationValidator consultationValidator = new ConsultationValidator(productServiceClient, linkItemQueryableRepo, new FileValidator(fileListMaxSize, fileListMaxCount));
        consultationApiController = new ConsultationApiController(new ReserveService(consultationValidator, fileUploadService, consultationRepo, memberQueryableRepo));
    }
    ...
    void 예약요청성공() throws Exception {
        ...
        linkItemSetUp.saveConsultation(consulter);
        ...
        //then
        ReservedItem reservedItemByProductId = linkItemSetUp.findReservedItemByProductId(reservationInput.getProductId());
        Assertions.assertEquals(reservationInput.getConsultantId(), reservedItemByProductId.getConsultant().getId());
    }
